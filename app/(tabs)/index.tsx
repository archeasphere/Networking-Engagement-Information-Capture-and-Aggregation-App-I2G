...
// Month name mapping for search functionality
const monthMappings = {
  'jan': 'january',
  'feb': 'february',
  'mar': 'march',
...
function HomeScreen({ }) {
  ...
  // Function to normalize month names for searching
  const normalizeMonthName = (dateString) => {
    // First, create a lowercase version of the date string
    const lowerCaseDate = dateString.toLowerCase();
    
    // Check for each abbreviated month and replace with full name
    let normalizedDate = lowerCaseDate;
    Object.entries(monthMappings).forEach(([abbr, full]) => {
      // Create regex that matches the abbreviation as a word
      const abbrRegex = new RegExp(`\\b${abbr}\\b`, 'g');
      normalizedDate = normalizedDate.replace(abbrRegex, full);
    });
  ...
  const filteredFiles = files.filter((file) => {
    ...
    const normalizedTimestamp = normalizeMonthName(file.timestamp);
    
    return (
      file.name.toLowerCase().includes(query) ||
      normalizedTimestamp.includes(query) ||
  ...
  const filteredLabels = labels.filter((label) => {
    const query = searchQuery.toLowerCase();
    const normalizedTimestamp = normalizeMonthName(label.timestamp);
